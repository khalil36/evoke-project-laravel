...
@php 
// var_dump($campaigns_created); //exit();
    @endphp
...
<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8 bg-gray-100">
	<div id="Q1XHz6LvJ27kEctjKjDb" class="md:grid md:grid-cols-3 md:gap-6">
		...
		<div class="mt-5 md:mt-0 md:col-span-2">
			<form method="POST" action="{{route('map-npis')}}" enctype="multipart/form-data" class="bg-gray-100" onsubmit="">
				{{ csrf_field() }}
				<div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
					<div class="grid grid-cols-6 gap-6">
						<div class="col-span-6 sm:col-span-4">
							...
							<input type="file" name="csv_file" id="csv_file" class="space-y-6 block" {{($campaigns_created) ? 'disabled' : '' }}>
                        ...
                        <label class="block font-medium text-sm text-gray-700 space-y-6 block mt-5" for="csv_file">
                          Import Method
                        </label>

                      <fieldset>
                        <div class="relative z-0 mt-1 border border-gray-200 rounded-lg cursor-pointer" x-data="{importMethod: '{{($campaigns_created) ? '' : 'merge' }}'}" >
                            <label type="button" class="cursor-pointer relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 border-t border-gray-200 rounded-t-none  rounded-b-none text-left">
                                <input {{($campaigns_created) ? 'disabled' : '' }} type="radio" x-model="importMethod" name="import_option" value="merge" class="sr-only" aria-labelledby="import_option-0-label" aria-describedby="import_option-0-description-0 import_option-0-description-1">
                            ...
                            <label type="button" class="cursor-pointer relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 border-t border-gray-200 rounded-t-none  rounded-b-none text-left">
                                <input {{($campaigns_created) ? 'disabled' : '' }} type="radio" x-model="importMethod" name="import_option" value="overwrite" class="sr-only" aria-labelledby="import_option-1-label" aria-describedby="import_option-1-description-0 import_option-1-description-1">
                            ...
                            <label type="button" class="cursor-pointer relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 border-t border-gray-200 rounded-t-none  rounded-b-none text-left">
                                <input {{($campaigns_created) ? 'disabled' : '' }} type="radio" x-model="importMethod" name="import_option" value="delete" class="sr-only" aria-labelledby="import_option-2-label" aria-describedby="import_option-2-description-0 import_option-2-description-1">
       ...
       <div class="flex items-center justify-end px-4 py-3 bg-gray-50 text-right sm:px-6 shadow sm:rounded-bl-md sm:rounded-br-md">
           <button {{($campaigns_created) ? 'disabled' : '' }} type="submit" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition">
              Map File
          </button>
      </div>

      <!-- This example requires Tailwind CSS v2.0+ -->
      <div class="bg-gray-50 sm:rounded-lg mt-5">
                ...
                <ul class="border border-gray-200 rounded-md divide-y divide-gray-200">
                    <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                      ...
                      <div class="ml-4 flex-shrink-0">
                        <a href="{{ config('APP_URL') }}/uploads/files/NPIS-example.csv" class="font-medium text-evoke-tertiary hover:text-evoke">
                            Download
                        </a>
                    </div>
                </li>
