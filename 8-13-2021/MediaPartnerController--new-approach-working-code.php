      ...
      //  exit();
        ...
        switch ($csvfile_data->data_import_type) {
            ...
            case 'append':
                ...
                $connection = SimpleExcelReader::create(storage_path('app/public/'.$csvfile_data->csv_filename));
                ...
                if (empty($file_has_column_header)) {
                    ...
                    $data_rows = array_merge([$headers],$rows->toArray());
                } else {
                    $rows = $connection->getRows();
                    $data_rows = $rows->toArray();
                }
                ...
                foreach ($data_rows as $row) { 
                    ...
                    foreach ($selected_values as $index => $field) {
                        ...
                        if ($field == 'date' ) {
                            if ($extension == 'csv') {
                                $date = strip_tags($row[$index]);
                            } else {
                                $date = $row[$index]->format('Y-m-d H:i:s');
                            }
                    ...
                    foreach ($selected_values as $index => $field) {
                        ...
                        if ($field == 'date' ) {
                            if ($extension == 'csv') {
                                ...
                                $MediaPartnerULD->$field = $row[$index]->format('Y-m-d H:i:s');
                            }
                        } else {
