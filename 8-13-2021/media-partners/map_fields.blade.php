            ...
            <table class="min-w-full divide-y divide-gray-200" style="width: 80%;">
              ...
              <tbody :class="hasColumnHeader == 'yes' ? '' : 'border-t-0'">
                @php
                  ...
                  $data_rows->take(2)->each(function(array $rowProperties, $count) {
                    echo '<tr class="'.( ($count%2==1) ? 'bg-gray-50' : 'bg-white') .'">';
                        foreach ($rowProperties as $key => $value):
                          if(gettype($value) == "object"){

                           // print_r($value['DateTime']->date);
                           echo $value->format('Y-m-d'); 
                           // var_dump( $value->getData());
                          //echo gettype($value);
                          }

                          echo '<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">'. gettype($value).'</td>';
