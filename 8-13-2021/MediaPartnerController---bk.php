...
class MediaPartnerController extends Controller
{
    ...
    */
    public function mapMediaPartnerULD(ValidateImportFile $request)
    {
       ...
       // echo "path: ".  $path . '-----'. $request->file('csv_file');// exit();
        ...
        $data_rows = $connection->take(2)->getRows();

        //$csv_data[] = (array_merge([$headers], $data_rows->toArray()) );
       // $csv_data[] = $data_rows->toArray();

        foreach ($data_rows->toArray() as $row) {   
            $csv_data[] = $row;
        } 

        // $fn = fn(array $rowProperties) => array_push($csv_data, $rowProperties);
        // $data_rows = $connection
        //     ->take(2)
        //     ->getRows()
        //     ->each($fn);
                //{
        ...
        // $csv_data = array_merge($headers, $data_rows);
         //dd($csv_data);
       //  echo '<pre>';
       //  print_r($csv_data); 
       // exit();
       ...
       // exit();
        ...
        return view('media-partners.map_fields', compact('csv_data', 'csv_data_file', 'media_partner_uld_fields', 'data_rows'));
