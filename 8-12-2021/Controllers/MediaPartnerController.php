    ...
    public function mapMediaPartnerULD(ValidateImportFile $request)
    {
        ...
        if ($extension == 'csv') {
            ...
            fclose($file);
             ...
             //echo $path . '----'.$file_name . '-----'.storage_path('app/public/'.$file_name);
            // $ppppp = Excel::import((string)storage_path('app/public/'.$file_name), PATHINFO_EXTENSION, function($file) {

            //     // modify stuff

            // })->convert('csv');

            // echo 'convert:'; print_r($ppppp); exit;

            //$file_data = Excel::import(new MediaPartnerULDImport, storage_path('app/public/'.$file_name));
            //Excel::import(new MediaPartnerULDImport, $request->file('csv_file'));
            // if (Session::has('xlsx_file_data')) {
            //     $file_data = Session::get('xlsx_file_data');
            // }
           // $rows = Excel::import(new MediaPartnerULDImport, $path);
       ...
       // echo "<pre>csv_data: "; print_r($csv_data); 
        // foreach ($csv_data as $value) {
        //     foreach($value as $key => $v){

        //         //echo 'key'. $key. ' value:'. pr$value;
        //         if ($key != 6) continue;
        //         echo '<br>value: '. $v ;
        //         // if (stistr($v, '44')) {
        //         //     // code...
        //         // }
        //         $date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject((int)$v)->format('Y-m-d H:i:s');//date("Y-m-d", strtotime($v));//strtotime($v);
        ...
        switch ($csvfile_data->data_import_type) {
            ...
            case 'append':
                ...
                if ($extension == 'csv') {
                    ...
                    $file_1 = fopen(storage_path('app/public/'.$csvfile_data->csv_filename), 'r');
                    while (($row = fgetcsv($file_1)) !== FALSE){

                        if ( ($file_has_column_header == 'yes' && $counter_1 > 0) || (empty($file_has_column_header) && $counter_1 >= 0) )
                        {
                    ...
                    $file_2 = fopen(storage_path('app/public/'.$csvfile_data->csv_filename), 'r');
                    while (($row = fgetcsv($file_2)) !== FALSE){
                        if ( ($file_has_column_header == 'yes' && $counter_2 > 0) || (empty($file_has_column_header) && $counter_2 >= 0) )
                        {
                            ...
                            foreach ($data as $row) {
                                if ( ($file_has_column_header == 'yes' && $counter_1 > 0) || (empty($file_has_column_header) && $counter_1 >= 0) )
                                {
                                    foreach ($selected_values as $index => $field) {
                                        ...
                                        if ($field == 'date' ) {
                                            $date =  \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject((int)$row[$index])->format('Y-m-d H:i:s');
                                            if(!strtotime($date)){
                                                 Session::flash('not_valid_dates', $row);
                                                 return redirect('/upload-media-partner-ulds')->with('count',  $counter_1 + 1);
                                            }
