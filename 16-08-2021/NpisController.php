    ...
    */
    ...
    public function processImport(Request $request)
    {
        ...
        if ($data_import_type == 'overwrite') {
            ...
            $rows = $connection
                ->getRows()
                ->each(function(array $row) use ($selected_values, $npi_index){
                $new_row = array_values($row);
                $existing_npi = Npis::where(['npi' => $new_row[$npi_index], 'team_id' =>Auth::user()->current_team_id])->first();
                ...
                if (!empty($existing_npi)) {
                   ...
                   foreach ($selected_values as $index => $field) {
                       ...
                       $existing_npi_2->$field = strip_tags((string)$new_row[$index]);
                   }
                   ...
                   foreach ($selected_values as $index => $field) {
                       if ($field == 'not_selected') continue;
                       $npis->$field = strip_tags((string)$new_row[$index]);
                   }
