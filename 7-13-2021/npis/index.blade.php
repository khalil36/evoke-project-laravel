...
@endphp
    ...
    <div class="grid bg-gray-100 grid-cols-2 gap-4 items-center">
      ...
      <div>
        <form method="GET" action="{{route('searchNPIs')}}">
          ...
          <div class="mt-1 flex rounded-md shadow-sm">
            <div class="relative flex items-stretch flex-grow focus-within:z-10">
              ...
              <input type="text" value="{{(isset($_GET['search_npis']) ? $_GET['search_npis'] : '')}}" name="search_npis" id="search_npis" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full rounded-none rounded-l-md pl-10 sm:text-sm border-gray-300" placeholder="Search NPIs">
            </div>
            ...
            @if($npis ?? '')
              <div class="p-2" id="npis-pagination">
                @if(isset($_GET['search_npis']))
                  {{ $npis->appends(['search_npis' => $_GET['search_npis'], 'search_npis_option' => $_GET['search_npis_option']])->links() }}
                @else
                  {{ $npis->links() }}
                @endif
              </div>
