...
class MediaPartnerController extends Controller
{
    ...
    */
    ...
    public function processUpload(Request $request)
    {
        ...
        $rows = $connection
            ->getRows()
            ->sole(function($row, $i) use ($date_column_index, $extension){
        ...
        if ($rows) {
            ...
            Session::flash('not_valid_dates', $rows);
            return redirect('/upload-media-partner-ulds')->with('count', 1);
            ...
            ->map(function (array $row) use ($extension, $date_column_index, $npi_column_index, $media_partner_id, $created_at, $updated_at){
                ...
                if ($extension == 'xlsx' && gettype( $new_row[$date_column_index] == 'object')) {
                    $date = $new_row[$date_column_index]->format('Y-m-d H:i:s');
                } else {
                    $date = date('Y-m-d H:i:s', strtotime(strip_tags($new_row[$date_column_index])));
                }
