                            ...
                            @foreach ($row as $key => $value)
                              @php
                                ...
                                if(gettype($value) == 'integer' || gettype($value) == 'double'){
                                  //$column =  \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($value)->format('Y-m-d');
                                }
                              @endphp
                                <th scope="col" :class="hasColumnHeader == 'yes' ? 'uppercase text-xs text-gray-500' : 'text-sm text-gray-900'" class="px-6 py-4 text-left font-medium tracking-wider">{{ $column }}</th>
                            @endforeach
                            </tr>
                            @php $count++ ; @endphp
                        @endif
                        ...
                        @foreach ($row as $key => $value)
                         @php
                             ...
                             //echo 'Type: '. gettype($value);
                              ...
                              if(gettype($value) == 'integer' || gettype($value) == 'double'){
                               //$column =  \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject((int)$value)->format('Y-m-d');
                               //$column = date('Y-m-d', $value);
                               //echo '<pre>Date:';print_r($date);
                              }
                            @endphp
